...
const HomeScreen = ({ navigation }: HomeScreenprops) => {
  ...
  const didInitialTemplateFetchRef = useRef(false);
  const didInitialSetupRef = useRef(false);
  ...
  const autoSelectFirstProfile = async (profiles: any[]) => {
    try {
      ...
      console.log('autoSelectFirstProfile error:', err);
    }
  };
  ...
  const fetchTemplateData = async (templateId?: string, index?: number) => {
    try {
      ...
      const already = renderTemplates[index]?.image_url;
      if (already) return;
        ...
        setRenderTemplates((prev) => {
          ...
          if (index >= copy.length) {
            for (let i = copy.length; i <= index; i++) {
              copy[i] = {
                id: templates[i]?.id ?? String(i),
                title: templates[i]?.title ?? null,
                image_url: null,
                description: null,
              };
            }
  ...
  useEffect(() => {
    if (didInitialSetupRef.current) return;
    ...
    const init = async () => {
      ...
      setTimeout(() => {
        if (templates.length > 0) {
          ...
          if (!renderTemplates[currentIndex]?.image_url && templates[currentIndex]?.id) {
            fetchTemplateData(templates[currentIndex].id, currentIndex);
            didInitialTemplateFetchRef.current = true;
          }
        }
      }, 0);
  ...
  useEffect(() => {
    ...
    if (!didInitialTemplateFetchRef.current) {
  ...
  const renderItem: FlatListProps<TemplateType>['renderItem'] = ({ item, index }) => {
    ...
    const isLoadingThisItem = loadingIndex === index && !imageUrl;
        ...
        <View style={styles.posterCard}>
          <View style={styles.templateContainer}>
            {isLoadingThisItem ? (
              ...
              <View style={styles.placeholderImage}>
                <Text style={{ color: '#000' }}>No Template Available</Text>
              </View>
            )}
          ...
          {templates.length === 0 && loading ? (
            ...
            <FlatList
              ...
              onMomentumScrollEnd={(e) => {
                ...
                if (!renderTemplates[index]?.image_url && templates[index]?.id) {
